...
# File upload
uploaded_file = st.file_uploader("Upload Mall Customers dataset (CSV)", type=["csv"])
...
if uploaded_file is not None:
    ...
    # Handle Gender/Genre encoding if present
    if "Gender" in df.columns:
        df["Gender"] = df["Gender"].map({"Male": 0, "Female": 1}).astype(int)
    elif "Genre" in df.columns:
        df["Genre"] = df["Genre"].map({"Male": 0, "Female": 1}).astype(int)
    else:
        st.warning("⚠️ No 'Gender' or 'Genre' column found in dataset. Skipping gender encoding.")
    ...
    st.info("👆 Please upload the Mall Customers dataset to get started.")
